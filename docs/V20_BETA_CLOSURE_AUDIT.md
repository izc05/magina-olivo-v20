...
Último HEAD de **código** completamente verificado en Foundation: `7d663b6e747abe201a5ffe6ca65e07dd8fa35ba5`.
...
Existe además el workflow manual `V20 staging deploy`, que exige SHA exacto perteneciente al candidate + los tres gates verdes del mismo SHA + confirmación `DEPLOY-STAGING`. Sube una release inmutable por SSH, valida `.env`, arranca PostgreSQL y crea **siempre** un backup `pg_dump -Fc` antes de migrar, despliega web/API/worker y ejecuta smoke HTTPS antes de marcar la release como actual.
...
- ✅ `V20 staging deploy` bloquea cualquier deploy si el SHA exacto no pertenece al candidate o no tiene candidate + E2E + readiness verdes.
- ✅ El deploy remoto crea siempre un backup inmediatamente antes de las migraciones y nunca restaura de forma destructiva automáticamente.
...
- Actions de CI/deploy fijadas a SHAs inmutables; Node proviene de `.nvmrc`.
