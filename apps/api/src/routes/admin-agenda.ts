...
async function readAdminTask(database: DatabaseClient, taskId: string) {
...
export function registerAdminAgendaRoutes(app: FastifyInstance, db: DatabaseClient | null) {
  ...
  app.post('/api/v1/admin/agenda', async (request, reply) => {
    ...
    `.execute(auth.database);
    const task = await readAdminTask(auth.database, id);
    if (!task) return reply.code(500).send({ error: 'scheduled_event_readback_failed' });
  ...
  app.patch('/api/v1/admin/agenda/:taskId', async (request, reply) => {
    ...
    const current = await readAdminTask(auth.database, params.data.taskId);
    ...
    await sql`
      ...
      SET title = COALESCE(${input.title ?? null}, title),
          ...
          task_kind = COALESCE(${input.task_kind ?? null}, task_kind),
    ...
    `.execute(auth.database);
    const task = await readAdminTask(auth.database, current.id);
    if (!task) return reply.code(500).send({ error: 'scheduled_event_readback_failed' });
