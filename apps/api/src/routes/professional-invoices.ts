...
export function registerProfessionalInvoiceRoutes(app: FastifyInstance, db: DatabaseClient | null) {
  ...
  app.post('/api/v1/professional/invoices', async (request, reply) => {
    ...
    const subtotalFromWorks = input.works.reduce((sum, item) => sum + item.amount_eur, 0);
    if (Math.abs(subtotalFromWorks - input.subtotal_eur) > 0.01) {
      return reply.code(400).send({ error: 'invoice_work_amounts_do_not_match_subtotal' });
    }
    ...
    if (works.rows.length !== ids.length) return reply.code(404).send({ error: 'work_not_found' });
  ...
  app.patch('/api/v1/professional/invoices/:invoiceId', async (request, reply) => {
    ...
    const existing = await sql<{ invoice_number: string | null; issued_on: string | null; status: string }>`
      SELECT invoice_number, issued_on::text, status
    ...
    if (nextStatus === 'issued' && (!nextNumber || !nextIssued)) {
      return reply.code(400).send({ error: 'issued_invoice_requires_number_and_date' });
    }
