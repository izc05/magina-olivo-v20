...
export function evaluateRadarObservationForTask(
  observation: RadarAgronomyObservation | null,
  task: AgronomyTask,
  now = new Date(),
) {
  if (!observation) return {
    ...
    summary: 'Sin observación radar disponible para la finca.',
    fresh: false,
    ageMinutes: null,
  };
  ...
  if (!fresh) return {
    ...
    suitability: 'unknown' as const,
    riskLevel: 'unknown' as const,
    summary: ageMinutes === null ? 'La fecha de la observación radar no es válida.' : `Observación radar antigua (${ageMinutes} min); no modifica la recomendación.`,
    fresh: false,
    ageMinutes,
  };

  if (observation.qualityFlags.length > 0 || observation.coverageStatus === 'outside' || observation.coverageStatus === 'unavailable') {
    return {
      actionable: false,
      suitability: 'unknown' as const,
      riskLevel: 'unknown' as const,
      summary: 'Radar sin calidad o cobertura suficiente para modificar la recomendación.',
      fresh,
      ageMinutes,
    };
  }

  if (observation.precipitationDetected !== true) return {
    actionable: false,
    suitability: 'good' as const,
    riskLevel: 'low' as const,
    summary: 'Sin eco de precipitación detectado cerca de la finca en la observación radar reciente.',
    fresh,
    ageMinutes,
  };
  ...
  if (task === 'harvest' || task === 'pruning' || task === 'work') {
    if (near || overOrVeryNear) return { actionable: true, suitability: 'caution' as const, riskLevel: 'medium' as const, summary: `Radar reciente: ${detail}.`, fresh, ageMinutes };
  }

  if (task === 'irrigation' && (near || overOrVeryNear)) {
    return { actionable: true, suitability: 'caution' as const, riskLevel: 'medium' as const, summary: `Radar reciente: ${detail}; conviene revisar la necesidad de riego.`, fresh, ageMinutes };
  }
...
export function combineAgronomySignals(
  ...
  if (!radar.actionable) return {
    suitability: forecast.suitability,
  ...
  return {
    suitability,
    riskLevel,
    summary: `${forecast.summary} · ${radar.summary}`,
    radarElevated: suitability !== forecast.suitability || riskLevel !== forecast.riskLevel,
  };
}
