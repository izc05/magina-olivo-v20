...
let unsafeConfigurationRejected = false;
try {
  assertSafeRuntimeEnvironment();
} catch (error) {
  unsafeConfigurationRejected = error instanceof Error && error.message.includes('unsafe_production_configuration');
}
requireCondition(unsafeConfigurationRejected, 'Production did not reject ALLOW_DEV_AUTH_HEADERS=true');

const db = createDatabase(databaseUrl);
const app = buildApp({ db });

try {
  const defensivePrivateRequest = await app.inject({
    method: 'GET',
    url: '/api/v1/me',
    headers: {
  ...
  requireCondition(
    defensivePrivateRequest.statusCode === 401,
    `Development auth headers bypassed production defense: ${defensivePrivateRequest.statusCode}`,
  );
  ...
  requireCondition(privateRequest.statusCode === 401, `Development auth headers were accepted in production mode: ${privateRequest.statusCode}`);
  ...
  console.log('Staging security smoke passed: unsafe dev-auth config fails closed; CORS, auth and private security headers remain enforced.');
} finally {
  await app.close();
  await db.destroy();
}
