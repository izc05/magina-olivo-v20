...
export type ExperienceSlot = { id:string; starts_at:string; ends_at:string|null; capacity:number; confirmed_count:number; price_override_cents:number|null; status:string; availablePlaces:number };
...
export type ExperienceAdminCatalog = {
  experiences:Array<Record<string,unknown> & {id:string;business_id:string;business_name:string;business_slug:string;slug:string;title:string;status:string;experience_type:ExperienceType}>;
  slots:Array<Record<string,unknown> & {id:string;experience_id:string;experience_title:string;business_name:string;starts_at:string;capacity:number;confirmed_count:number;status:string}>;
  bookings:Array<Record<string,unknown> & {id:string;experience_id:string;experience_title:string;business_name:string;contact_name:string;contact_email:string|null;contact_phone:string|null;party_size:number;status:BookingStatus;created_at:string;slot_starts_at:string|null}>;
};

export type MyExperienceCatalog = {
  role:'owner'|'manager'|'editor'|'analyst';
  experiences:ExperienceAdminCatalog['experiences'];
  slots:ExperienceAdminCatalog['slots'];
  bookings:ExperienceAdminCatalog['bookings'];
};
...
export async function requestExperienceBooking(businessSlug:string,slug:string,input:{slotId?:string|null;contactName:string;contactEmail?:string|null;contactPhone?:string|null;partySize:number;requestedFor?:string|null;message?:string|null;consentBusinessContact:true}){
...
export const experienceAdminApi={
  ...
  createSlot:(experienceId:string,input:{startsAt:string;endsAt?:string|null;capacity:number;priceOverrideCents?:number|null;status?:'open'|'full'|'cancelled'|'hidden'})=>apiFetch<{slot:{id:string}}>(`/api/v1/admin/business-experiences/${encodeURIComponent(experienceId)}/slots`,{method:'POST',body:JSON.stringify(input)}),
...
export const myExperienceApi={
  ...
  createSlot:(businessId:string,experienceId:string,input:{startsAt:string;endsAt?:string|null;capacity:number;priceOverrideCents?:number|null;status?:'open'|'full'|'cancelled'|'hidden'})=>apiFetch<{slot:{id:string}}>(`/api/v1/my/businesses/${encodeURIComponent(businessId)}/experiences/${encodeURIComponent(experienceId)}/slots`,{method:'POST',body:JSON.stringify(input)}),
