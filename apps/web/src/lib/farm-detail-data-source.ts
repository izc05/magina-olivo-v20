...
export type FarmActivityView = { id: string; date: string; title: string; summary?: string; domainType?: string; iconKey?: string };
...
export type FarmDetailData = { activity: FarmActivityView[]; harvest: FarmHarvestView; economics: FarmEconomicsView; data: FarmDataView; documents: FarmDocumentView[] };
...
export async function loadApiFarmDetailData(fieldId: string, workspaceId: string): Promise<FarmDetailData> {
  const [activity, harvest, harvestCommercial, economics, map, documents] = await Promise.all([
    ...
    apiFetch<ApiEconomicsPayload>(`/api/v1/fields/${encodeURIComponent(fieldId)}/economics-summary`, { workspaceId }),
  ...
  return {
    activity: activity.items.map((item) => ({ id: item.id, date: item.occurred_at.slice(0, 10), title: item.title, summary: item.summary ?? undefined, domainType: item.domain_type, iconKey: item.icon_key ?? undefined })),
    harvest: {
      totalKg: harvest.total_kg, weightedYieldPercent: harvest.weighted_yield_percent ?? undefined, pendingResults: harvest.pending_results,
      deliveries: harvest.deliveries.map((item) => ({ id: item.delivery_id, date: item.delivery_at.slice(0, 10), kg: item.kg, destination: item.cooperative_or_mill ?? undefined, ticketNumber: item.ticket_number ?? undefined, yieldPercent: item.yield_percent ?? undefined, resultDate: item.result_date ?? undefined })),
...
export function loadPreviewFarmDetailData(farmId: string, source?: string | null): FarmDetailData {
  const deliveries = getLocalHarvestDeliveries(undefined, farmId); const results = getLocalHarvestResults(); const parcels = getLocalParcels(farmId);
  const deliveryViews: FarmHarvestDeliveryView[] = deliveries.map((delivery) => { const allocation = delivery.allocations.find((item) => item.farmId === farmId); const kg = allocation?.kg ?? (allocation?.percentage !== undefined ? delivery.totalKg * allocation.percentage / 100 : delivery.allocations.length === 1 ? delivery.totalKg : 0); const result = results.filter((item) => item.deliveryId === delivery.id).sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0]; return { id: delivery.id, date: delivery.deliveredAt.slice(0, 10), kg, destination: delivery.millOrCooperativeName, ticketNumber: delivery.ticketNumber, yieldPercent: result?.yieldPercent, resultDate: result?.resultDate }; });
  if (source === 'demo' && farmId === 'las-cenillas' && deliveryViews.length === 0) deliveryViews.push({ id: 'demo-delivery', date: '2026-12-12', kg: demoDelivery.kilograms, destination: demoDelivery.cooperative, ticketNumber: demoDelivery.ticketNumber });
  const totalKg = deliveryViews.reduce((sum, item) => sum + item.kg, 0); const withResult = deliveryViews.filter((item) => item.yieldPercent !== undefined && item.kg > 0); const resultKg = withResult.reduce((sum, item) => sum + item.kg, 0); const weightedYieldPercent = resultKg > 0 ? withResult.reduce((sum, item) => sum + item.kg * (item.yieldPercent ?? 0), 0) / resultKg : undefined;
  return { activity: [], harvest: { totalKg, weightedYieldPercent, pendingResults: deliveryViews.filter((item) => item.yieldPercent === undefined).length, deliveries: deliveryViews.sort((a, b) => b.date.localeCompare(a.date)), accruedEur: 0, collectedEur: 0, pendingEur: 0, settlements: [] }, economics: emptyEconomics(), data: { parcelCount: parcels.length, references: parcels.map((parcel) => ({ id: parcel.id, source: parcel.kind, reference: parcel.cadastralReference, areaHa: parcel.areaHa, status: parcel.status })) }, documents: [] };
}

export function emptyFarmDetailData(): FarmDetailData { return { activity: [], harvest: { totalKg: 0, pendingResults: 0, deliveries: [], accruedEur: 0, collectedEur: 0, pendingEur: 0, settlements: [] }, economics: emptyEconomics(), data: { parcelCount: 0, references: [] }, documents: [] }; }
