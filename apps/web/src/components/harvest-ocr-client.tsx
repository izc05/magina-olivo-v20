...
export function HarvestOcrClient() {
  ...
  async function saveRemote(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (sourceDocumentId) {
        try {
          await linkDocumentToDomain({ workspaceId: selectedWorkspaceId, documentId: sourceDocumentId, fieldId: context.id, domainType: 'harvest_delivery', domainRecordId: response.delivery.id });
          setSourceLinked(true);
        } catch (linkError) {
          console.warn('Delivery saved but source document link failed', linkError);
          setSourceLinkWarning('La entrega se ha guardado, pero el albarán no pudo enlazarse automáticamente. El documento sigue conservado en la finca.');
        }
      }

      if (plannedEventId) {
        try {
          await completePlannedTask({ workspaceId: selectedWorkspaceId, taskId: plannedEventId, domainType: 'harvest_delivery', domainRecordId: response.delivery.id });
  ...
  if (apiMode) {
    return <><header className="page-title"><span className="eyebrow dark">MI CAMPO · COSECHA · {context.name.toUpperCase()}</span><h1>Registrar entrega</h1><p>Primero guardamos peso y albarán. El rendimiento se incorpora cuando lo comunique la cooperativa o almazara.</p></header>
      ...
      <form className="quick-record-form" onSubmit={saveRemote}><section className="card record-panel"><div className="record-fields">
