...
export function NewFarmWizard() {
  ...
  function finish() {
    ...
    saveLocalField({ id, name: name.trim() || 'Nueva finca', municipality: municipality.trim() || undefined, oliveTrees: trees ? Number(trees) : undefined, variety, waterRegime, createdAt: new Date().toISOString() });
    setSavedId(id);
    setStep(3);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }
  ...
  if (step === 3) {
    const fieldHref = savedId ? `/mi-campo/fincas/local?id=${encodeURIComponent(savedId)}` : '/mi-campo';
    ...
    return <section className="card new-farm-success">
      <div className="success-mark"><SproutIcon /></div><span className="eyebrow dark">GUARDADA EN ESTE DISPOSITIVO</span><h1>{name || 'Nueva finca'}</h1>
      <p>{trees || '—'} olivas · La finca ya tiene ficha propia y puede guardar sus trabajos de forma independiente.</p>
      <div className="success-effects"><span>✓ Finca guardada localmente</span><span>✓ Ficha propia creada</span><span>{linked ? '✓ Localización de demo asociada' : '○ Terreno oficial pendiente, opcional'}</span></div>
      <div className="record-actions"><Link href={fieldHref} className="secondary-action action-link">Ver ficha</Link><Link href={registerHref} className="primary action-link">Registrar trabajo <ArrowIcon /></Link></div>
    </section>;
  }

  if (step === 2) {
    return <div className="new-farm-location-flow">
      <section className="card new-farm-summary"><span className="new-farm-tree"><SproutIcon /></span><div><small>NUEVA FINCA</small><strong>{name}</strong><span>{trees} olivas · {municipality}</span></div><button onClick={() => setStep(1)}>Editar</button></section>
      <section className="card locate-panel"><span className="eyebrow dark">PASO 2 · UBICACIÓN</span><h2>¿Quieres localizarla ahora?</h2><p>Es recomendable, pero no obligatorio. Puedes hacerlo más tarde desde Datos → Terreno.</p><div className="locate-grid">{[
        ['mapa','📍','Buscar en mapa','Toca la finca sobre el mapa'],['catastro','▦','Catastro','Buscar por referencia catastral'],['sigpac','▱','SIGPAC','Polígono y parcela agrícola'],['dibujar','✎','Dibujar','Marca tú mismo el contorno'],
      ...
      <div className="new-farm-actions"><button className="secondary-action" type="button" onClick={finish}>Ahora no</button><button className="primary" type="button" onClick={finish} disabled={!mode}>Guardar finca →</button></div>
    </div>;
  }
