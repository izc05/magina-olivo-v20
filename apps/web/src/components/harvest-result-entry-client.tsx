...
export function HarvestResultEntryClient() {
  ...
  useEffect(() => {
    ...
    apiFetch<HarvestSummaryResponse>(`/api/v1/fields/${encodeURIComponent(context.id)}/harvest-summary`, { workspaceId: selectedWorkspaceId })
      ...
      .catch((cause) => {
        console.error('Unable to load harvest deliveries', cause);
        if (!cancelled) setError('No se han podido cargar las entregas de esta finca.');
      })
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!resultDate || !Number.isFinite(yieldPercent) || yieldPercent <= 0) {
      setError('Indica una entrega, fecha y rendimiento válidos.');
      return;
    }
    try {
      setSaving(true);
      setError(null);
      await apiFetch(`/api/v1/deliveries/${encodeURIComponent(deliveryId)}/results`, {
        method: 'POST',
  ...
  return <>
    <header className="page-title"><span className="eyebrow dark">MI CAMPO · COSECHA · {context.name.toUpperCase()}</span><h1>Registrar rendimiento</h1><p>Selecciona expresamente la entrega a la que pertenece el resultado.</p></header>
    ...
    <form className="quick-record-form" onSubmit={submit}>
      ...
      <section className="record-save-bar"><small>El resultado quedará separado de la entrega, liquidación y cobro.</small><button className="primary" type="submit" disabled={!deliveryId || saving}>{saving ? 'Guardando…' : 'Guardar rendimiento →'}</button></section>
    </form>
  </>;
}
