...
export function LocalFieldModulePage() {
  ...
  const [summary, setSummary] = useState({ activities: 0, costs: 0, planned: 0, harvests: 0 });
  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      setSummary({
        ...
        harvests: activities.filter((item) => item.type === 'harvest').length,
      });
    }
  }, []);
  ...
  return <>
    ...
    {view === 'cosechas' && <section className="card module-highlight harvest-highlight"><span>COSECHAS LOCALES</span><strong>{summary.harvests}</strong><p>Confirmadas desde el OCR de demostración en este dispositivo.</p></section>}
