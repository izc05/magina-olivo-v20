...
export function DocumentReviewClient() {
  ...
  async function startOcr() {
    ...
    try { setRunning(true); setError(null); await requestDocumentOcr(selectedWorkspaceId, documentId, analysis.version.id); await refresh(); }
    catch (cause) { console.error('Unable to start OCR', cause); setError('No se ha podido iniciar el análisis OCR.'); }
    finally { setRunning(false); }
  }
  async function confirmReview() {
    if (!selectedWorkspaceId || !analysis?.extraction) return;
    const confirmed: Record<string, unknown> = {}; const corrections: Record<string, unknown> = {};
    for (const [key, raw] of Object.entries(values)) {
      const value = coerceValue(key, raw); confirmed[key] = value;
      if (JSON.stringify(value) !== JSON.stringify(analysis.extraction.data_json[key])) corrections[key] = value;
    }
    try { setSaving(true); setError(null); await confirmDocumentExtraction(selectedWorkspaceId, analysis.extraction.id, confirmed, corrections); await refresh(); }
    catch (cause) { console.error('Unable to confirm OCR review', cause); setError('No se ha podido guardar la revisión.'); }
  ...
  return <>
    ...
    <section className="section card"><div className="section-head"><h2>Estado OCR</h2><span>{analysis.ocr?.status ?? 'sin analizar'}</span></div><p>El OCR solo propone datos. Nada se convierte en gasto, entrega, rendimiento, liquidación o cobro hasta que lo revises y confirmes expresamente.</p>
      ...
      {analysis.ocr?.status === 'succeeded' && !analysis.extraction ? <p>El texto se ha leído, pero no se han identificado campos estructurados con suficiente patrón. El documento sigue siendo válido.</p> : null}
    </section>
