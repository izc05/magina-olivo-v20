...
export function AttentionSummaryCard({ fieldId, compact = false }: { fieldId?: string; compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    loadAttentionSummary({ workspaceId: selectedWorkspaceId, fieldId, limit: compact ? 3 : 5 })
      .then((result) => { if (!cancelled) setData(result); })
      .catch((error) => { console.warn('Attention summary unavailable', error); if (!cancelled) setData(null); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [apiConfigured, compact, fieldId, selectedWorkspaceId, status]);
  ...
  return <section className="section">
    <div className="section-head"><h2>{fieldId ? 'Lo importante ahora' : 'Tu campo hoy'}</h2><Link href="/mi-campo/hoy">Ver agenda</Link></div>
    <div className="card feed today-list">
      {data.items.map((item) => <div className="feed-row" key={item.id}>
        <div className="feed-copy">
          <strong>{item.title}</strong>
          <small>{fieldId ? item.scheduledAt.slice(0, 16).replace('T', ' ') : `${item.fieldName} · ${item.scheduledAt.slice(0, 16).replace('T', ' ')}`}</small>
          {item.advisory ? <small>
