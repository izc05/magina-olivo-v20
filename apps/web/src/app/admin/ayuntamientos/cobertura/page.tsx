...
function buildChecks(municipality: Municipality, counts: Counts) { const directory = municipality.directory; return [
...
export default function AdminMunicipalityCoveragePage() {
  ...
  const allRows = useMemo<CoverageRow[]>(() => municipalities.map((municipality) => { const counts = entryCounts(entries, municipality.id); const checks = buildChecks(municipality, counts); return { municipality, counts, checks, completed: checks.filter((check) => check.ok).length }; }).sort((a, b) => a.completed - b.completed || a.municipality.name.localeCompare(b.municipality.name, 'es')), [municipalities, entries]);
  ...
  const summary = useMemo(() => ({ complete: allRows.filter((row) => row.completed === 10).length, withEconomy: allRows.filter((row) => row.counts.mill + row.counts.directory > 0).length, withCurrent: allRows.filter((row) => row.counts.news + row.counts.event > 0).length, missingTourism: allRows.filter((row) => !row.municipality.directory?.tourism_url).length }), [allRows]);
  ...
  return <main className="admin-shell">
    <header className="admin-topbar"><div><a href={municipalityAdminHref('/admin/ayuntamientos', selectedMunicipality?.slug)}>← Ayuntamientos</a><h1>Cobertura municipal</h1><p>Control operativo de los 16 municipios. Son diez comprobaciones explícitas, no una puntuación editorial subjetiva.</p></div><div className={styles.actions}><a href={municipalityAdminHref('/admin/ayuntamientos/actualidad', selectedMunicipality?.slug)}>Actualidad</a><a href="/admin/web">CMS ↗</a></div></header>
