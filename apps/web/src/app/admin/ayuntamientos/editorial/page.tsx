...
function textField(entry: CmsEntry, key: string) { const value = asObject(entry.content_json)[key]; return typeof value === 'string' ? value : ''; }
function roleOf(entry: CmsEntry): MunicipalityRole { const role = textField(entry, 'municipality_role'); return role === 'profile' || role === 'heritage' || role === 'nature' || role === 'tourism' ? role : ''; }
function roleLabel(role: MunicipalityRole) { if (role === 'profile') return 'Perfil principal'; if (role === 'heritage') return 'Patrimonio'; if (role === 'nature') return 'Naturaleza'; if (role === 'tourism') return 'Turismo'; return 'Sin función'; }
function updatePayload(entry: CmsEntry, patch: Partial<Pick<CmsEntry, 'summary' | 'status' | 'featured' | 'media_url' | 'sort_order'>> = {}, contentJson = entry.content_json) {
  return { type: entry.type, title: entry.title, slug: entry.slug, summary: patch.summary !== undefined ? patch.summary : entry.summary, content_json: contentJson, status: patch.status ?? entry.status, featured: patch.featured ?? entry.featured, starts_at: entry.starts_at, ends_at: entry.ends_at, media_url: patch.media_url !== undefined ? patch.media_url : entry.media_url, external_url: entry.external_url, sort_order: patch.sort_order ?? entry.sort_order };
}
...
export default function MunicipalityEditorialAdminPage() {
  ...
  useEffect(() => { if (auth.status === 'authenticated') void load().catch(() => setError('No se ha podido cargar la configuración editorial municipal.')); }, [auth.status, load]);
  ...
  function changeMunicipality(id: string) {
  ...
  async function saveEntry(entry: CmsEntry, patch: Partial<Pick<CmsEntry, 'summary' | 'status' | 'featured' | 'media_url' | 'sort_order'>>, success: string) {
    ...
    try { await adminApi.updateContent(entry.id, updatePayload(entry, patch)); await load(); setMessage(success); }
    catch { setError('No se ha podido guardar la decisión editorial.'); }
  ...
  async function setProfile(next: CmsEntry) {
    ...
    try {
      if (profile && profile.id !== next.id) { const previous = { ...asObject(profile.content_json) }; delete previous.municipality_role; await adminApi.updateContent(profile.id, updatePayload(profile, {}, previous)); }
      ...
      await load(); setMessage(`${next.title} es ahora el perfil principal de ${selectedMunicipality.name}.`);
    } catch { setError('No se ha podido cambiar el perfil principal.'); }
    finally { setBusyId(null); }
  }

  async function toggleFeatured(entry: CmsEntry, nextFeatured: boolean) {
    if (nextFeatured && !entry.featured && featuredCount >= 3) { setError('Solo puede haber tres imprescindibles destacados. Desmarca uno antes de añadir otro.'); return; }
  ...
  return <main className="admin-shell">
    <header className="admin-topbar">
      <div><a href={municipalityAdminHref('/admin/ayuntamientos', selectedMunicipality?.slug)}>← Centro de control municipal</a><h1>Portada editorial municipal</h1><p>Controla perfil, imagen hero y “Lo imprescindible” con contenido CMS real.</p></div>
    ...
    <MunicipalityAdminNav slug={selectedMunicipality?.slug} name={selectedMunicipality?.name} active="portada" />

    <section className={`admin-card ${styles.selectorCard}`}>
      <label><span>Municipio</span><select value={municipalityId} onChange={(event) => changeMunicipality(event.target.value)}>{municipalities.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <div><strong>{municipalEntries.length}</strong><span>lugares CMS vinculados</span></div><div><strong>{featuredCount}/3</strong><span>imprescindibles elegidos</span></div>
    </section>

    {selectedMunicipality ? <>
      <section className={`admin-card ${styles.profileCard}`}>
        <div className={styles.sectionHeading}><div><span className={styles.kicker}>HERO Y PERFIL</span><h2>Perfil principal de {selectedMunicipality.name}</h2><p>Su imagen alimenta el hero público y su resumen presenta el municipio.</p></div><a href={municipalityAdminHref('/admin/ayuntamientos/patrimonio', selectedMunicipality.slug)}>Clasificación avanzada →</a></div>
        ...
        </div> : <div className={styles.empty}><strong>Este municipio todavía no tiene perfil principal.</strong><p>Selecciona uno de sus lugares vinculados para convertirlo en perfil.</p></div>}
        <div className={styles.profileCandidates}>{municipalEntries.map((entry) => <button key={entry.id} type="button" disabled={busyId === `profile:${entry.id}` || profile?.id === entry.id} onClick={() => void setProfile(entry)}><span>{roleLabel(roleOf(entry))}</span><strong>{entry.title}</strong><small>{profile?.id === entry.id ? 'Perfil actual' : 'Usar como perfil principal'}</small></button>)}{!municipalEntries.length ? <p>No hay lugares CMS vinculados a este municipio.</p> : null}</div>
      </section>

      <section className="admin-card">
        <div className={styles.sectionHeading}><div><span className={styles.kicker}>LO IMPRESCINDIBLE</span><h2>Destacados de portada</h2><p>Máximo tres. La prioridad más alta aparece primero.</p></div><strong>{featuredCount}/3 seleccionados</strong></div>
        <div className={styles.discoveryList}>{discoveries.map((entry) => <article key={entry.id} className={entry.featured ? styles.discoveryFeatured : styles.discoveryRow}>
        ...
        </article>)}{!discoveries.length ? <div className={styles.empty}><strong>Sin descubrimientos clasificados.</strong><p>Usa Patrimonio y turismo para vincular contenido como patrimonio, naturaleza o turismo.</p></div> : null}</div>
      </section>
    </> : null}
  </main>;
}
