...
type FormState = { id: string | null; municipalityId: string; role: MunicipalityRole; title: string; slug: string; summary: string; body: string; sourceUrl: string; sourceLabel: string; verifiedAt: string; externalUrl: string; mediaUrl: string; status: CmsEntryStatus; featured: boolean; sortOrder: number; };
const ROLE_LABELS: Record<MunicipalityRole, string> = { profile: 'Perfil principal', heritage: 'Patrimonio', nature: 'Naturaleza', tourism: 'Turismo' };
...
function roleOf(entry: CmsEntry): MunicipalityRole | null { const role = field(entry, 'municipality_role'); return role === 'profile' || role === 'heritage' || role === 'nature' || role === 'tourism' ? role : null; }
function slugify(value: string) { return value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''); }
function mediaPublicUrl(asset: AdminMediaAsset) { return `${apiBaseUrl}${asset.public_path}`; }
function blankForm(municipalityId = ''): FormState { return { id: null, municipalityId, role: 'heritage', title: '', slug: '', summary: '', body: '', sourceUrl: '', sourceLabel: '', verifiedAt: new Date().toISOString().slice(0, 10), externalUrl: '', mediaUrl: '', status: 'draft', featured: false, sortOrder: 0 }; }
...
export default function AdminMunicipalityContentEditorPage() {
  ...
  const load = useCallback(async () => {
    ...
    setForm((current) => {
      if (current.municipalityId) return current;
      const requested = readMunicipalitySlug();
      const next = activeMunicipalities.find((item) => requested && item.slug === requested) ?? activeMunicipalities[0];
      if (next) replaceMunicipalityContext(next.slug);
      return { ...current, municipalityId: next?.id ?? '' };
    });
  }, []);

  useEffect(() => { if (auth.status === 'authenticated') void load().catch(() => setError('No se ha podido cargar el editor municipal.')); }, [auth.status, load]);

  const municipality = useMemo(() => municipalities.find((item) => item.id === form.municipalityId) ?? null, [municipalities, form.municipalityId]);
  const municipalityEntries = useMemo(() => entries.filter((entry) => field(entry, 'municipality_id') === form.municipalityId).sort((a, b) => Number(b.featured) - Number(a.featured) || b.sort_order - a.sort_order || a.title.localeCompare(b.title, 'es')), [entries, form.municipalityId]);
  const visibleEntries = useMemo(() => { const needle = query.trim().toLocaleLowerCase('es'); return municipalityEntries.filter((entry) => !needle || `${entry.title} ${entry.summary ?? ''} ${ROLE_LABELS[roleOf(entry) ?? 'heritage']}`.toLocaleLowerCase('es').includes(needle)); }, [municipalityEntries, query]);
  ...
  function changeMunicipality(id: string, preserveForm = false) {
    const next = municipalities.find((item) => item.id === id);
    replaceMunicipalityContext(next?.slug);
    setForm((current) => preserveForm ? { ...current, municipalityId: id } : blankForm(id));
    setMessage(null); setError(null);
  }

  function startNew() { setForm(blankForm(form.municipalityId)); setMessage(null); setError(null); }
  function edit(entry: CmsEntry) {
    const content = asObject(entry.content_json);
    const entryMunicipalityId = field(entry, 'municipality_id');
    const nextMunicipality = municipalities.find((item) => item.id === entryMunicipalityId);
    replaceMunicipalityContext(nextMunicipality?.slug);
    setForm({ id: entry.id, municipalityId: entryMunicipalityId, role: roleOf(entry) ?? 'heritage', title: entry.title, slug: entry.slug, summary: entry.summary ?? '', body: typeof content.body === 'string' ? content.body : '', sourceUrl: field(entry, 'source_url'), sourceLabel: field(entry, 'source_label'), verifiedAt: field(entry, 'verified_at') || new Date().toISOString().slice(0, 10), externalUrl: entry.external_url ?? '', mediaUrl: entry.media_url ?? '', status: entry.status, featured: entry.featured, sortOrder: entry.sort_order });
    setMessage(null); setError(null);
  }
  function changeTitle(title: string) { setForm((current) => ({ ...current, title, slug: current.id ? current.slug : slugify(`${municipality?.slug ?? 'municipio'}-${title}`) })); }
  ...
  async function ensureSingleProfile(targetId: string | null) {
    ...
    for (const entry of others) { const next = { ...asObject(entry.content_json) }; delete next.municipality_role; await adminApi.updateContent(entry.id, { type: entry.type, title: entry.title, slug: entry.slug, summary: entry.summary, content_json: next, status: entry.status, featured: entry.featured, starts_at: entry.starts_at, ends_at: entry.ends_at, media_url: entry.media_url, external_url: entry.external_url, sort_order: entry.sort_order }); }
  }
  ...
  async function save() {
    ...
    try {
      ...
      const contentJson = { ...existingContent, municipality_id: municipality.id, municipality_name: municipality.name, municipality_slug: municipality.slug, municipality_role: form.role, body: form.body.trim(), source_url: form.sourceUrl.trim(), source_label: form.sourceLabel.trim(), verified_at: form.verifiedAt };
      const payload = { type: 'place' as const, title: form.title.trim(), slug: form.slug.trim(), summary: form.summary.trim() || null, content_json: contentJson, status: form.status, featured: form.role === 'profile' ? false : form.featured, starts_at: existing?.starts_at ?? null, ends_at: existing?.ends_at ?? null, media_url: form.mediaUrl.trim() || null, external_url: form.externalUrl.trim() || null, sort_order: form.sortOrder };
      if (form.id) await adminApi.updateContent(form.id, payload); else await adminApi.createContent(payload);
      await load(); setForm(blankForm(municipality.id)); replaceMunicipalityContext(municipality.slug);
      setMessage(form.id ? 'Contenido municipal actualizado y auditado.' : 'Contenido municipal creado correctamente.');
    } catch { setError('No se ha podido guardar el contenido. Revisa los datos y vuelve a intentarlo.'); }
  ...
  return <main className="admin-shell">
    <header className="admin-topbar">
      <div><a href={municipalityAdminHref('/admin/ayuntamientos', municipality?.slug)}>← Ayuntamientos</a><h1>Editor de contenido municipal</h1><p>Crea y edita perfiles, patrimonio, naturaleza y turismo sin tocar código.</p></div>
    ...
    <MunicipalityAdminNav slug={municipality?.slug} name={municipality?.name} active="contenido" />
    ...
    <section className={styles.layout}>
      <aside className={`admin-card ${styles.sidebar}`}>
        ...
        <label>Municipio<select value={form.municipalityId} onChange={(event) => changeMunicipality(event.target.value)}>{municipalities.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      ...
      <div className={styles.mainColumn}>
        <section className={`admin-card ${styles.editorCard}`}>
          ...
          <div className={styles.grid}>
            <label>Municipio<select value={form.municipalityId} onChange={(e) => changeMunicipality(e.target.value, true)}>{municipalities.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
