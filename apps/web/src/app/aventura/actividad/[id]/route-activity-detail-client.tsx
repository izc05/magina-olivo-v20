...
export function RouteActivityDetailClient({ activityId }: { activityId: string }) {
  const [track, setTrack] = useState<RouteActivityTrack | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<'auth' | 'not_found' | 'generic' | null>(null);
  ...
  if (error) return <main className={styles.shell}><Link href="/aventura/actividad" className={styles.back}>← Actividad</Link><section className={styles.card}>
    <h1>{error === 'auth' ? 'Necesitas iniciar sesión' : error === 'not_found' ? 'Recorrido no encontrado' : 'No se puede abrir el recorrido'}</h1>
    <p>{error === 'auth' ? 'Este mapa contiene datos privados de ubicación y solo lo puede consultar su propietario.' : error === 'not_found' ? 'Puede haberse eliminado o no pertenecer a tu cuenta.' : 'Comprueba la conexión y vuelve a intentarlo.'}</p>
    {error === 'auth' ? <Link className={styles.login} href={`/login?next=${encodeURIComponent(`/aventura/actividad/${activityId}`)}`}>Iniciar sesión →</Link> : null}
  </section></main>;
