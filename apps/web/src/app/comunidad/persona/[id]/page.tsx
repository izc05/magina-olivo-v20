...
const roleLabels: Record<string, string> = {
  agricultor: 'Agricultor/a', propietario: 'Propietario/a', trabajador: 'Trabajador/a',
  profesional_agricola: 'Profesional agrícola', tecnico: 'Técnico/a', empresa: 'Empresa', otro: 'Miembro de Mágina',
};
...
export default function CommunityMemberPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setNotFound(false);
    apiFetch<{ member: CommunityMember }>(`/api/v1/public/community/members/${encodeURIComponent(params.id)}`)
  ...
  return <main className="app-shell">
    ...
    <div className="page">
      <section className={styles.shell}>
        ...
        {!loading && !member ? <div className={styles.state}><h1>{notFound ? 'Perfil no público' : 'Perfil no disponible'}</h1><p>Esta ficha solo existe cuando la persona ha elegido expresamente un perfil público en Mágina Olivo.</p></div> : null}
        {member ? <>
