...
export function RouteAdventurePanel({ routeId, slug }: { routeId: string; slug: string }) {
  ...
  const collectionKinds = (['landmark', 'trivia', 'observation', 'photo', 'collection', 'rest'] as const)
    .map((kind) => {
      ...
      return { kind, total: checkpoints.length, unlocked: checkpoints.filter((checkpoint) => unlocked.has(checkpoint.id)).length };
    })
  ...
  const albumCategories = (['flora', 'fauna', 'heritage', 'olive_culture', 'tradition', 'landscape'] as const)
    .map((category) => {
      ...
      return { category, total: checkpoints.length, unlocked: checkpoints.filter((checkpoint) => unlocked.has(checkpoint.id)).length };
    })
    .filter((entry) => entry.total > 0);
  ...
  return <section className={styles.communitySection} aria-labelledby="route-adventure-title">
    ...
    <article className={styles.infoCard} aria-labelledby="route-adventure-journal-title">
      <div className={styles.reviewMeta}><strong id="route-adventure-journal-title">Cuaderno del Explorador</strong><span>{effectivePercent}% descubierto</span></div>
      <h3>{explorerRank(effectivePercent, completed)}</h3>
      <p>{progress?.run ? `Has descubierto ${effectiveUnlocked} de ${effectiveTotal} elementos de esta aventura.` : `Esta ruta guarda ${effectiveTotal} descubrimientos. Inicia la aventura para ir incorporándolos a tu cuaderno.`}</p>
      <div className={styles.infoGrid}>{collectionKinds.map((entry) => <div className={styles.infoCard} key={entry.kind}><strong>{checkpointIcon(entry.kind)} {entry.unlocked}/{entry.total}</strong><p>{checkpointKind(entry.kind)}</p></div>)}</div>
      {albumCategories.length ? <><h3>Álbum de esta ruta</h3><div className={styles.infoGrid}>{albumCategories.map((entry) => <div className={styles.infoCard} key={entry.category}><strong>{entry.unlocked}/{entry.total}</strong><p>{albumCategoryLabel(entry.category)}</p></div>)}</div></> : null}
      <div className={styles.actionRow} aria-label="Colección de esta ruta">
        {definition.checkpoints.map((checkpoint, index) => {
          const done = unlocked.has(checkpoint.id); const locked = isLocked(index);
          return <button key={checkpoint.id} type="button" className={styles.secondaryAction} title={done ? checkpoint.title : locked ? `Etapa ${index + 1} bloqueada por progresión` : checkpoint.title} onClick={() => { if (!locked) focusCheckpoint(checkpoint); }}>{done ? `✓ ${checkpoint.title}` : locked ? `🔒 Etapa ${index + 1}` : `✦ Etapa ${index + 1}`}</button>;
        })}
      </div>
    </article>

    {progress?.badges.length ? <div className={styles.actionRow} aria-label="Insignias conseguidas">{progress.badges.map((badge) => <span key={badge} className={styles.secondaryAction}>{badgeLabel(badge)}</span>)}</div> : null}
    {authRequired ? <div className={styles.infoCard}><h3>Guarda tu aventura</h3><p>Puedes ver los retos sin cuenta, pero para iniciar el recorrido y conservar XP necesitas iniciar sesión.</p><Link className={styles.primaryAction} href={`/login?next=${encodeURIComponent(`/rutas/detalle?slug=${slug}`)}`}>Iniciar sesión</Link></div> : null}
    {!progress?.run && !authRequired ? <div className={styles.actionRow}><button className={styles.primaryAction} type="button" disabled={busy !== null} onClick={start}>{busy === 'start' ? 'Iniciando…' : 'Iniciar Modo Aventura'}</button></div> : null}
    {message ? <p role="status" aria-live="polite">{message}</p> : null}

    <div className={styles.reviewGrid}>
      {definition.checkpoints.map((checkpoint, index) => {
        const done = unlocked.has(checkpoint.id); const locked = isLocked(index);
        const options = Array.isArray(checkpoint.answer_options) ? checkpoint.answer_options.filter((option) => option && typeof option.key === 'string' && typeof option.label === 'string') : [];
        return <article className={styles.reviewCard} key={checkpoint.id} aria-disabled={locked}>
          <div className={styles.reviewMeta}><strong>{done ? '✓ ' : locked ? '🔒 ' : ''}Etapa {index + 1} · {checkpointKind(checkpoint.kind)}</strong><span>{checkpoint.is_required ? 'Obligatorio' : 'Extra'} · {checkpoint.points} XP</span></div>
          ...
          {checkpoint.question && !done && !locked ? <fieldset style={{ border: 0, padding: 0, margin: '16px 0 0' }}><legend><strong>{checkpoint.question}</strong></legend>{options.map((option) => <label key={option.key} style={{ display: 'block', marginTop: 8 }}><input type="radio" name={`answer-${checkpoint.id}`} value={option.key} checked={answers[checkpoint.id] === option.key} onChange={() => setAnswers((current) => ({ ...current, [checkpoint.id]: option.key }))} />{' '}{option.label}</label>)}{checkpoint.hint ? <small style={{ display: 'block', marginTop: 8 }}>Pista: {checkpoint.hint}</small> : null}</fieldset> : null}
          <div className={styles.actionRow} style={{ marginTop: 12 }}>{!locked ? <button className={styles.secondaryAction} type="button" onClick={() => focusCheckpoint(checkpoint)}>Ver punto en mapa</button> : null}{active && !done && !locked ? <button className={styles.primaryAction} type="button" disabled={busy !== null} onClick={() => unlock(checkpoint)}>{busy === checkpoint.id ? 'Comprobando…' : 'Estoy aquí'}</button> : null}{done ? <span>Desbloqueado</span> : locked ? <span>Bloqueado</span> : null}</div>
        </article>;
      })}
    </div>
    {active && progress.stats.required_remaining === 0 ? <div className={styles.actionRow}><button className={styles.primaryAction} type="button" disabled={busy !== null} onClick={complete}>{busy === 'complete' ? 'Cerrando aventura…' : 'Completar aventura'}</button></div> : null}
    {completed ? <div className={styles.infoCard}><h3>Recorrido lúdico completado</h3><p>{adventure.completion_message ?? 'Has desbloqueado todos los checkpoints necesarios de esta aventura.'}</p><p><small>Esto certifica el progreso del mini‑juego, no una validación oficial de actividad deportiva ni del estado del sendero.</small></p></div> : null}
  </section>;
}
