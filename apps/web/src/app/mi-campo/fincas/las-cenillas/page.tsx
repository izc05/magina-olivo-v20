...
import { ArrowIcon, CalendarIcon, CampaignIcon, CostIcon, DocumentIcon, MapPinIcon, PlusIcon, PruningIcon, SproutIcon, TerritoryMapIcon, TreatmentIcon, WaterIcon } from '@/components/icons';
import { lasCenillas } from '@/lib/demo-data';

const base = '/mi-campo/fincas/las-cenillas';
const modules = [
  [CampaignIcon,'Cosecha',`${lasCenillas.campaign} · ${lasCenillas.campaignKg.toLocaleString('es-ES')} kg`,`${base}/cosechas`],
  [WaterIcon,'Riegos',`Próximo · ${lasCenillas.nextIrrigationLong}`,`${base}/riegos`],
  [TreatmentIcon,'Tratamientos',`Último · ${lasCenillas.lastTreatment}`,`${base}/tratamientos`],
  [SproutIcon,'Abonado',`Último · ${lasCenillas.lastFertilization}`,`${base}/abonos`],
  [PruningIcon,'Poda',`Última · ${lasCenillas.lastPruning}`,`${base}/poda`],
  [CostIcon,'Costes',`${lasCenillas.campaignCostEur} € · campaña`,`${base}/gastos`],
  [CalendarIcon,'Calendario','Próximo · riego 14 sep',`${base}/calendario`],
  [CalendarIcon,'Historia','Todo lo realizado',`${base}/historia`],
  [DocumentIcon,'Documentos',`${lasCenillas.documents} archivos`,`${base}/documentos`],
  [TerritoryMapIcon,'Terreno','Referencias por validar',`${base}/terreno`],
] as const;
...
export default function FarmPage(){
  return <main className="app-shell"><Topbar/><div className="page farm-detail-page">
    ...
    <section className="section"><div className="section-head"><h2>Últimos movimientos</h2><Link href={`${base}/historia`} className="detail-link">Ver todos <ArrowIcon/></Link></div><div className="farm-timeline">
      <article className="timeline-row"><span className="timeline-node harvest"><CampaignIcon /></span><time>12 dic 2026</time><div><strong>Entrega de cosecha</strong><small>1.842 kg · SCA San Isidro</small></div><ArrowIcon/></article>
      <article className="timeline-row"><span className="timeline-node treatment"><TreatmentIcon /></span><time>18 ago 2026</time><div><strong>Tratamiento fitosanitario</strong><small>Mosca del olivo · documento guardado</small></div><ArrowIcon/></article>
      <article className="timeline-row"><span className="timeline-node pruning"><PruningIcon /></span><time>14 feb 2026</time><div><strong>Poda de mantenimiento</strong><small>23 olivas · trabajo finalizado</small></div><ArrowIcon/></article>
    </div></section>
