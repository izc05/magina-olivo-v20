...
function Invoke-LocalPsqlFile {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "No se pudo aplicar $(Split-Path -Leaf $Path)."
  }
}

if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
  throw 'Docker Desktop no está instalado o no está disponible en PATH. Instálalo e inicia Docker Desktop antes de ejecutar este script.'
}
