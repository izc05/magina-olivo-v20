...
test('planifica una tarea y solo la completa al registrar el trabajo real', async ({ page }) => {
  ...
  await page.getByLabel('Tarea').fill('Desbroce planificado E2E');
  ...
  const taskCard = page.locator('article').filter({ hasText: 'Desbroce planificado E2E' }).first();
  ...
  await page.locator('input[name="title"]').fill('Desbroce realizado E2E');
  ...
  const completedCard = page.locator('article').filter({ hasText: 'Desbroce planificado E2E' }).first();
  ...
  await expect(page.getByTitle(`${farmName}: Desbroce planificado E2E`)).toBeVisible();
});
